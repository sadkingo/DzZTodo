<tr x-data="{ modalIsOpen : false }">
    ...
    @if(getCrudConfig('Category')->delete or getCrudConfig('Category')->update)
        <td>
            ...
            @if(getCrudConfig('Category')->update && hasPermission(getRouteName().'.category.update', 1, 1, $category))
                <a href="@route(getRouteName().'.category.update', $category->id)" class="btn text-primary mt-1">
                    <i class="icon-pencil"></i>
                </a>
            @endif

            @if(getCrudConfig('Category')->delete && hasPermission(getRouteName().'.category.delete', 1, 1, $category))
