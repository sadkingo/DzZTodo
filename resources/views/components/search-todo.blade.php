...
<div class="flex mb-8">
    <label for="search-dropdown" class="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-white">Your
        Email</label>
    <button id="dropdown-button" id="dropdownDelayButton" data-dropdown-toggle="dropdownDelay" data-dropdown-delay="50" data-dropdown-trigger="hover"
        class="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-900 bg-gray-100 border border-gray-300 rounded-l-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700 dark:text-white dark:border-gray-600"
        type="button">{{ ($categorySearch ?? 'All')?($categorySearch ?? 'All'): 'All'  }}<svg class="w-2.5 h-2.5 ml-2.5" aria-hidden="true"
    ...
    <div id="dropdownDelay" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">
            <li>
                <button wire:click.prevent="categorySearch('0')" type="button"
                    class="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">All</button>
                @foreach ($categories as $item)
                    <button wire:click.prevent="categorySearch('{{ $item->name }}')" type="button"
    ...
    <div class="relative w-full">
        <input type="search" id="search-dropdown" wire:model='search'
            ...
            placeholder="Search for tasks" required>
        <button type="submit"
